    ...
    public class MatchValidator : AbstractValidator<Match>
    {
        public MatchValidator(ValidationStates state = ValidationStates.Add)
        {
            ...
            if (state == ValidationStates.Update)
            {
                ...
                RuleFor(x => x.Score1).NotNull().NotEmpty().When(x => x.MatchTime < DateTime.Now).WithMessage("Maç oynanmış, skor bilgisi girilmesi gereklidir.");
                ...
                RuleFor(x => x.Score1).Null().When(x => x.MatchTime > DateTime.Now).WithMessage("Oynanmamış maça skor giremezsiniz");
                RuleFor(x => x.Score2).Null().When(x => x.MatchTime > DateTime.Now).WithMessage("Oynanmamış maça skor giremezsiniz");
